...
- Cuando se agrega más de una transacción relacionada a la vez (no solo una), conviene tratarlas como un mismo paquete de cambio en PFCG/Fiori/ChaRM en lugar de repetir el ciclo completo por cada una. Y si el rol es compuesto (agrupa varios roles simples), actualizar el rol simple no basta por sí solo para usuarios nuevos: hay que confirmar que el rol compuesto quede asignado explícitamente a cada usuario que lo necesite, no asumir que "ya que el rol base cambió, listo".
